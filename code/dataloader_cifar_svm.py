            ...
            if self.mode == 'all':
                ...
                if self.mode == "labeled":
                    ...
                    for i in range(num_class):
                        ...
                        # GMM Selection
                        prob_gmm = np.argsort(-1 * probability[class_indices])
                    ...
                    pred_idx_svm = pred.nonzero()[0]

                    if len(pred_idx_svm) > 0:
                        # 将 pred_idx_gmm 和 pred_idx_svm 连接成一个数组
                        pred_idx = np.concatenate([pred_idx_gmm, pred_idx_svm], axis=0)

                        # 将结果数组变平成一维
                        pred_idx = np.ravel(pred_idx)
                        print("pred_idx.shape", pred_idx.shape)

                        # pred_idx = pred_idx_gmm + pred_idx_svm
                        pred_idx = list(set(pred_idx))

                    else:
                        pred_idx = pred_idx_gmm
...
class cifar_dataloader():  
    ...
    def run(self, sample_ratio=0, mode='train', pred=[], prob=[], class_flex_adjust=[], gmm_prob=[]):
        if mode=='warmup':
